Convert any URL to an Image using a publicly accessible webpage URL or one behind basic authentication or cookie authentication using our authentication mechanisms.

## Endpoints

### GET /url/image

Supports simple requests with only two parameter options.

- `url` (required)
- `timeout` (optional)

```html title="GET /url/image"
https://api.cloudlayer.io/v1/url/image?url={url}&timeout={timeout}
```

### POST /url/image

Supports all parameters being passed into request as JSON. All parameter types are supported.

- [Url Parameters](#url-parameters)
- [Base Parameters](#base-parameters)

:::tip
Make sure your `Content-Type` gets set to `application/json`, and that you have placed your `X-API-Key` header with your API Key from your account.
:::

```html title="POST /url/image"
https://api.cloudlayer.io/v1/url/image
```

```json title="JSON Payload"
{
  "url": "https://google.com"
}
```

### Examples

```json title="Viewport Example"
{
  "url": "https://google.com",
  "viewPort": {
    "width": 800,
    "height": 400
  }
}
```

```shell title="cURL Example"
curl --request POST \
  --url https://api.cloudlayer.io/v1/url/image \
  --header 'Content-Type: application/json' \
  --header 'x-api-key: <YOUR-API-KEY>' \
  --data '{
	"url": "https://google.com"
}' \
--output "google.png"
```

```shell title="Wget Example"
wget --method POST \
  --header 'Content-Type: application/json' \
  --header 'x-api-key: <YOUR-API-KEY>' \
  --body-data '{"url": "https://google.com"}' \
  - https://api.cloudlayer.io/v1/url/image \
  -O "google.png"
```

:::info
For more examples, checkout the [Url to Image Examples](examples/url-to-image/).
:::
